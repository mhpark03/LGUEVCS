...
* netsh http add urlacl url=http://*:8180/ user=everyone
*/
...
namespace WindowsFormsApp2
{
    public partial class Form1 : Form
    {
        ...
        string statusTruri = "/api/v1/OCPP/statusNotification/";
        string startTruri = "/api/v1/OCPP/startTransaction/";
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            listViewEV.View = View.Details;
            listViewEV.GridLines = true;
            listViewEV.FullRowSelect = true;
            listViewEV.CheckBoxes = false;

            listViewEV.Columns.Add("시간", 60, HorizontalAlignment.Center);
            listViewEV.Columns.Add("state", 50, HorizontalAlignment.Center);
            listViewEV.Columns.Add("TR", 30, HorizontalAlignment.Center);
...
*/                  
            ...
            if ((rxMsg != "\r") && (rxMsg != "\n"))
            {
                ...
                else if (rxMsg.StartsWith("$OM_R_RCIN_RSP=", System.StringComparison.CurrentCultureIgnoreCase))
                {
                    ...
                    //{
                        ...
                        if (charger.tab == "server")
                        {
                            int offset = rx_svrdatas[0].Length + rx_svrdatas[1].Length + 2;
                            Log("R" + "  " + str2.Substring(offset));
                            parseRemoteCMD(str2.Substring(offset));
                        }
                    //}
                    //else
                    //{
                ...
                else if (textBox70.Text != string.Empty && rxMsg.StartsWith(textBox70.Text))           // Module FOTA 이벤트
                {
                    ...
                    if (Altair.Checked == false)
                    {
                        ...
                        else if (dev.model.StartsWith("BG96"))
                        {
                            string[] state = str2.Replace("\"", string.Empty).Split(',');
                            if (state[0] == "EVENT: 2")
                            {
                                logPrintInTextBox("Bootstrap started", "");
                                endLwM2MTC("tc0202", string.Empty, string.Empty, string.Empty, string.Empty);
                            ...
                            else if (state[0] == "EVENT: 4")
                            {
                                logPrintInTextBox("registration started", "");
                            }
                            else if (state[0] == "EVENT: 5")
                            {
                                logPrintInTextBox("registration completed", "");
                            }
                            else if (state[0] == "EVENT: 7")
                            {
                                logPrintInTextBox("module fota downloading", "");
                            }
                            else if (state[0] == "EVENT: 8")
                            {
                                logPrintInTextBox("module fota update", "");
                            }
                            else if (state[0].StartsWith("OBSERVE:"))
                            {
                                if (state[1] == "/26241/0/0")
                                {
                                    logPrintInTextBox("26241 object subscription completed", "");
                                    endLwM2MTC("tc0301", string.Empty, string.Empty, string.Empty, string.Empty);
        ...
        private void parseNextReceiveData(string str2)
        {
            ...
            switch (state)
            {
                ...
                case states.getimsi:
                    textBox87.Text = str2;
                    if (str2.StartsWith("45006"))
                    {
                        string ctn = "0" + str2.Substring(5, str2.Length - 5);
        ...
        private void button63_Click(object sender, EventArgs e)
        {
            ...
            if (saveFile.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    ...
                    worksheet.Cells[i, 4] = new Cell(checkBox12.Text);
                    i++;
            ...
            if (ofd.FileName.Length > 0)
            {
                try
                {
                    ...
                    if (worksheet.Name == "options" && worksheet.Cells[0, 0].ToString() == button73.Text)
                    {
                        ...
                        checkBox12.Text = worksheet.Cells[i, 4].ToString();
                        if (checkBox12.Text == "\"")
        ...
        private void button97_Click(object sender, EventArgs e)
        {
            ...
            {
                this.sendDataOut("at%setacfg=Identification.Model.Model,"+ tBoxDeviceModel.Text);
                lbActionState.Text = states.setmbspmodel.ToString();
            }
        }
        ...
        private void btnLwM2MData_Click(object sender, EventArgs e)
        {
            if (svr.enrmtKeyId != string.Empty)
            {
                if (lbDevEntityId.Text == "." && tbDevID.Text==string.Empty)
                {
                    MessageBox.Show("CTN이 등록되어 있지 않습니다.확인이 필요합니다.");
                }
                else
        ...
        private void SendDataToLwM2M(object param)
        {
            ...
            ReqHeader header = new ReqHeader();
            if (tbDevID.Text == string.Empty)
                setDeviceEntityID();
            header.Url = brkUrlL + "/" + tbDevID.Text + "/10250/0/1";
    ...
    private void btnDeviceStatusCheck_Click(object sender, EventArgs e)
        ...
        private void RetriveDataToLwM2M(object param)
        {
            ...
            if (tbDevID.Text == string.Empty)
                setDeviceEntityID();
            header.Url = brkUrlL + "/" + tbDevID.Text + "/10250/0/0";
...
*/
        ...
        private void tabControl1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else if (tabControl1.SelectedTab.Name == "tabEVDevice")
            {
                charger.tab = "device";
            }
            else if (tabControl1.SelectedTab.Name == "tabECServer")
            {
                charger.tab = "server";
            }
        }
        ...
        private void receiveDataAltair(string rcvData)
        {
            string[] rx_svrdatas = rcvData.Split(',');    // 수신한 데이터를 한 문장씩 나누어 array에 저장

            if (rx_svrdatas[0] == "WRITE")
            {
                if (rx_svrdatas[3] == "/10250/0/1")
                {
                    //received hex data make to ascii code
                    lbLwM2MRcvData.Text = BcdToString(rx_svrdatas[4].ToCharArray());
                    if (lbLwM2MRcvData.Text == label40.Text)
                        endLwM2MTC("tc0502", string.Empty, string.Empty, string.Empty, string.Empty);
                    else
                        endLwM2MTC("tc0502", string.Empty, "20000100", lbLwM2MRcvData.Text, string.Empty);
                ...
                else if (rx_svrdatas[3] == "/26241/0/1")
                {
                    timer2.Stop();

                    int dataSize = rx_svrdatas[4].Length / 2;
                    // Firmware File Information Block Checking
                    if ((device_total_index == "0") && (dataSize == 8))
                    {
                        if (rx_svrdatas[4].Substring(0, 4) == "0000")
                        {
                            device_total_index = rx_svrdatas[4].Substring(4, 4);
                            device_fota_checksum = rx_svrdatas[4].Substring(8, 8);
                            logPrintInTextBox("total Index= " + device_total_index + ", checksum = " + device_fota_checksum + "를 수신하였습니다.", "");
                            if (lbActionState.Text == states.lwm2mtc06032.ToString())
                                lbActionState.Text = states.lwm2mtc06033.ToString();
                        }
                    ...
                    {
                        device_fota_index = rx_svrdatas[4].Substring(0, 4);
                        //tBoxFOTAIndex.Text = device_fota_index;
                        string checksum = rx_svrdatas[4].Substring(4, 8);
                        logPrintInTextBox("index= " + device_fota_index + "/" + device_total_index + ", checksum= " + checksum + "를 수신하였습니다.", "");
            ...
            else if (rx_svrdatas[0] == "OBSERVE_START")
            {
                if (rx_svrdatas[5] == "/10250/0/0")
                {
                    altdataid = rx_svrdatas[3];
                    logPrintInTextBox("10250 object subscription completed", "");
                    if (lbActionState.Text == states.lwm2mtc02011.ToString() || lbActionState.Text == states.lwm2mtc02012.ToString())
                    {
                        timer2.Stop();
                ...
                else if (rx_svrdatas[5] == "/26241/0/0")
                {
                    altfotaid = rx_svrdatas[3];
                    logPrintInTextBox("26241 object subscription completed", "");
                    endLwM2MTC("tc0301", string.Empty, string.Empty, string.Empty, string.Empty);
...
*/
        ...
        private void evTcpDisconnect()
        {
            isRunning = false;
            button38.Text = "TCP 연결";
        ...
        private void evTcpConnect()
        {
            try
            {
                server = new TcpClient(textBox13.Text, Int32.Parse(textBox8.Text));
                //server = new TcpClient("192.168.10.254", 9000);
                ns = server.GetStream();
                isRunning = true;
                button38.Text = "TCP 해제";
                Thread recvThread = new Thread(new ThreadStart(RecvThread));
                recvThread.Start();
                Console.WriteLine("TCP 연결이 성공하였습니다.");
            }
            catch (SocketException)
            {
                MessageBox.Show("TCP 연결에 실패하였습니다.");
            }
        }
        ...
        private void Log(string msg)
        {
            BeginInvoke(new Action(() =>
            {
                ...
                if (charger.tab == "server")
                {
                    listViewEVSP.Items.Add(newitem);
                    if (listViewEVSP.Items.Count > 35)
                    {
                        if (listViewEVSP.Items.Count > 500)
                            listViewEVSP.Items.RemoveAt(0);

                        listViewEVSP.TopItem = listViewEVSP.Items[listViewEVSP.Items.Count - 1];
                    }
                }
                else
                {
                    listViewEV.Items.Add(newitem);
                    if (listViewEV.Items.Count > 35)
                    {
                        if (listViewEV.Items.Count > 500)
                            listViewEV.Items.RemoveAt(0);

                        listViewEV.TopItem = listViewEV.Items[listViewEV.Items.Count - 1];
                    }
                }
            }));
        }
        ...
        public void RecvThread()
        {
            ...
            while (isRunning)
            {
                try
                {
                    ...
                    foreach (byte _eachChar in buffer)
                    {
                        int value = Convert.ToInt32(_eachChar);
                        hexOutput += String.Format("{0:X2}", value);
                    }

                    Console.WriteLine("TCP data receive : " + hexOutput);

                    if (hexOutput.Length > 0)
                    {
                        if (hexOutput.StartsWith("02"))
                        {
                            hexOutput = hexOutput.Substring(6);
                            string cmd = hexOutput.Substring(0, 2);
                            string rcvlen = hexOutput.Substring(2, 4);
                            string rcvdata = BcdToString(hexOutput.Substring(6, Convert.ToInt16(rcvlen, 16) * 2).ToCharArray());
                            hexOutput = hexOutput.Substring(6 + Convert.ToInt16(rcvlen, 16) * 2 + 4);
                            if (cmd == "80")
                            {
                                Console.WriteLine("oneM2M 연결 설정 완료 !!!");
                                if (hexOutput.StartsWith("03"))
                                {
                                    if (charger.tab == "server")
                                ...
                                {
                                    Console.WriteLine("No End character : " + hexOutput);
                                }
                            }
                            else if (cmd == "81")
                            {
                                Console.WriteLine("LTE RF 상태 조회 !!!");
                                if (hexOutput.StartsWith("03"))
                                {
                                    if (charger.tab == "server")
                                        listViewEVSP.Invoke(new LogToForm(LogEVSP), new object[] { "R" + cmd + rcvdata });
                            ...
                            {
                                Console.WriteLine("Remote Control 수신 !!!");
                                if (hexOutput.StartsWith("03"))
                                {
                                    if (charger.tab == "server")
                                        listViewEVSP.Invoke(new LogToForm(LogEVSP), new object[] { "R" + cmd + rcvdata });
                            ...
                            Console.WriteLine("OCPP data receive : " + rcvdata);
                        }
                        else
                        {
                            Console.WriteLine("No Start Character !!!");
                        }
                    }
                }
                catch (Exception ex)
        ...
        private void parseRemoteCMD(string rcvdata)
        {
            rcvdata = rcvdata.Replace("&quot;", "\"");

            JObject obj = JObject.Parse(rcvdata);
            string cmd = (obj["command"] ?? " ").ToString();
            charger.logid = (obj["logid"] ?? " ").ToString();
            ...
            switch (cmd)
            {
                case "RemoteStartTransaction":
                    ...
                    textBox15.Invoke(new LogToForm(setParam), new object[] {"1"+ idtag });
                ...
                case "UpdateFirmware":
                    ...
                    textBox11.Invoke(new LogToForm(setParam), new object[] { "4" + location });
                ...
                case "GetDiagnostics":
                    ...
                    textBox16.Invoke(new LogToForm(setParam), new object[] { "5" + loc });
        ...
        private void parseRSRP(string rcvdata)
        {
            JObject obj = JObject.Parse(rcvdata);
            charger.rssi = (obj["rssi"] ?? " ").ToString();
            charger.rsrp = (obj["rsrp"] ?? " ").ToString();
            charger.snr = (obj["snr"] ?? " ").ToString();
        }

        private void parseOneM2M(string rcvdata)
        {
            JObject obj = JObject.Parse(rcvdata);
            charger.entityId = (obj["entityid"] ?? " ").ToString();
            charger.imsi = (obj["imsi"] ?? " ").ToString();
            charger.iccid = (obj["iccid"] ?? " ").ToString();
            charger.rssi = (obj["rssi"] ?? " ").ToString();
            charger.rsrp = (obj["rsrp"] ?? " ").ToString();
            charger.snr = (obj["snr"] ?? " ").ToString();
        }

        private void SendingTCPStream(string kind, string strBuffer)
        {
            int checksum = 0;
            string hexOutput = "02";

            checksum += tcpindex / 256;
            checksum += tcpindex % 256;
            tcpindex++;
            hexOutput += String.Format("{0:x4}", tcpindex);

            int value = Convert.ToInt32(kind);
            checksum += value;
            hexOutput += String.Format("{0:x2}", value);

            int len = strBuffer.Length;
            checksum += len;
            hexOutput += String.Format("{0:x4}", len);

            foreach (char _eachChar in strBuffer)
            {
            ...
            hexOutput += String.Format("{0:x4}", checksum);
            hexOutput += "03";

            byte[] send = Encoding.ASCII.GetBytes(BcdToString(hexOutput.ToCharArray())); // 아스키 인코더로 stringTobyte[]

            ns.Write(send, 0, send.Length);
            ns.Flush(); // 버퍼 제거

            Console.WriteLine("TCP Send = "+ hexOutput);
            Log("T" + kind + strBuffer);
        }

        private void btnOneM2M_Click(object sender, EventArgs e)
        {
            if (isRunning == true)
                SendingTCPStream("00", "{\"chargerid\":\"" + textBox14.Text + "\"}");
            else
                MessageBox.Show("TCP 연결 상태를 확인하세요.");
        }
        ...
        private string SendDataToEVSP(string uri, string data)
        {
            ReqHeader header = new ReqHeader();
            ...
            string retStr = SendEVRequest(header, data);
            return retStr;
        }

        public string SendEVRequest(ReqHeader header, string data)
        {
            string resResult = string.Empty;
            ...
            try
            {
                ...
                if (header.ContentType != string.Empty)
                    wReq.ContentType = header.ContentType;
                if (header.X_M2M_RI != string.Empty)
                    wReq.Headers.Add("X-EVC-RI", header.X_EVC_RI);
                if (header.X_M2M_Origin != string.Empty)
                    wReq.Headers.Add("X-EVC-BOX", header.X_EVC_BOX);
                wReq.Headers.Add("X-EVC-MDL", "UMT100");
                wReq.Headers.Add("X-EVC-OS", "Windows10");
                wReq.Headers.Add("X-EVC-SN", textBox7.Text);
            ...
            catch (WebException ex)
            {
                ...
                {
                    var resp = (HttpWebResponse)ex.Response;
                    Log("R" + "  " + "HTTP/1.1 " + (int)resp.StatusCode + " " + resp.StatusCode.ToString());
                    Console.WriteLine("HTTP/1.1 " + (int)resp.StatusCode + " " + resp.StatusCode.ToString());
                    Console.WriteLine("");
                    for (int i = 0; i < resp.Headers.Count; ++i)
                        Console.WriteLine(" " + resp.Headers.Keys[i] + ": " + resp.Headers[i]);
        ...
        private void listViewEV_SelectedIndexChanged(object sender, EventArgs e)
        {
            ListView.SelectedListViewItemCollection itemColl = listViewEV.SelectedItems;
            foreach (ListViewItem item in itemColl)
            {
        ...
        private void btnbootstatus_Click(object sender, EventArgs e)
        {
            evcarStatusNT("Available");
        }
        ...
        private void btntriff_Click(object sender, EventArgs e)
        {
            evcarTariff();
        }
        ...
        private void SendDataToEVC()
        {
            ...
            string packetStr = "<m2m:cin xmlns:m2m=\"http://www.onem2m.org/xml/protocols\">";
            packetStr += "<cnf>text/json</cnf>";
            string data = charger.txdata;
            data = data.Replace("&quot;", "\"");
            packetStr += "<con>" + data + "</con>";
            packetStr += "</m2m:cin>";

            string retStr = SendHttpEVCMD(header, packetStr);
            //if (retStr != string.Empty)
            //    LogWrite(retStr);
        }

        public string SendHttpEVCMD(ReqHeader header, string data)
        {
            string resResult = string.Empty;
            ...
            try
            {
                ...
                if (charger.tab == "server")
                    listViewEVSP.Invoke(new LogToForm(LogEVSP), new object[] { "T" + "  " + wReq.Method + " " + wReq.RequestUri });
                else
                    listViewEV.Invoke(new LogToForm(Log), new object[] { "T" + "  " + wReq.Method + " " + wReq.RequestUri });
                Console.WriteLine(wReq.Method + " " + wReq.RequestUri + " HTTP/1.1");
                Console.WriteLine("");
                for (int i = 0; i < wReq.Headers.Count; ++i)
                    Console.WriteLine(wReq.Headers.Keys[i] + ": " + wReq.Headers[i]);
                ...
                {
                    if (charger.tab == "server")
                        listViewEVSP.Invoke(new LogToForm(LogEVSP), new object[] { "T" + "  " + data });
                    else
                        listViewEV.Invoke(new LogToForm(Log), new object[] { "T" + "  " + data });
                ...
                {
                    if (charger.tab == "server")
                        listViewEVSP.Invoke(new LogToForm(LogEVSP), new object[] { "T" + "  " + (int)wRes.StatusCode + " " + wRes.StatusCode.ToString() });
                    else
                        listViewEV.Invoke(new LogToForm(Log), new object[] { "T" + "  " + (int)wRes.StatusCode + " " + wRes.StatusCode.ToString() });
                    Console.WriteLine("HTTP/1.1 " + (int)wRes.StatusCode + " " + wRes.StatusCode.ToString());
                    Console.WriteLine("");
                    for (int i = 0; i < wRes.Headers.Count; ++i)
                        Console.WriteLine("[" + wRes.Headers.Keys[i] + "] " + wRes.Headers[i]);
            ...
            catch (WebException ex)
            {
                if (ex.Status == WebExceptionStatus.ProtocolError && ex.Response != null)
                {
                    var resp = (HttpWebResponse)ex.Response;
                    if (charger.tab == "server")
                        listViewEVSP.Invoke(new LogToForm(LogEVSP), new object[] { "R" + "  " + (int)resp.StatusCode + " " + resp.StatusCode.ToString() });
                    else
                        listViewEV.Invoke(new LogToForm(Log), new object[] { "R" + "  " + (int)resp.StatusCode + " " + resp.StatusCode.ToString() });
                    Console.WriteLine("HTTP/1.1 " + (int)resp.StatusCode + " " + resp.StatusCode.ToString());
                    Console.WriteLine("");
                    for (int i = 0; i < resp.Headers.Count; ++i)
                        Console.WriteLine(" " + resp.Headers.Keys[i] + ": " + resp.Headers[i]);
        ...
        private void LogEVSP(string msg)
        {
            BeginInvoke(new Action(() =>
            {
                string time = DateTime.Now.ToString("hh:mm:ss");
        ...
        private void listViewEVSP_SelectedIndexChanged(object sender, EventArgs e)
        {
            ListView.SelectedListViewItemCollection itemColl = listViewEVSP.SelectedItems;
            foreach (ListViewItem item in itemColl)
            {
                string data = item.SubItems[4].Text;
                if (data.StartsWith("<m2m"))
                {
                    XmlDocument xDoc = new XmlDocument();
                    xDoc.LoadXml(data);
                    StringWriter writer = new StringWriter();
        ...
        private void btnRemoteStop_Click(object sender, EventArgs e)
        {
            if (svr.enrmtKeyId != string.Empty)
            {
                if (charger.entityId != string.Empty)
                {
                    ...
                    var json = new JObject();
                    json.Add("command", "RemoteStopTransaction");
                    json.Add("transactionId", Convert.ToUInt64(textBox9.Text));
                    json.Add("logid", charger.logid);
                    charger.txdata = json.ToString((Newtonsoft.Json.Formatting)Formatting.None);
                    //charger.txdata = json.ToString();
        ...
        private void button131_Click_1(object sender, EventArgs e)
        {
            if (svr.enrmtKeyId != string.Empty)
            {
                if (charger.entityId != string.Empty)
                {
                    ...
                    if (isRunning == true)
                        evTcpDisconnect();          // TCP Disconnection
                    evTcpConnect();
                    if (isRunning == true)      // oneM2M EntityID read
                    {
                        SendingTCPStream("00", "{\"chargerid\":\"" + textBox14.Text + "\"}");
                    ...
                    charger.logid = DateTime.Now.ToString("yyyyMMddHHmmss") + "_admmode";
        ...
        private void button84_Click_1(object sender, EventArgs e)
        {
            if (svr.enrmtKeyId != string.Empty)
            {
                if (charger.entityId != string.Empty)
                {
                    ...
                    var json = new JObject();
                    json.Add("command", "GetDiagnostics");
                    json.Add("location", textBox16.Text);
                    json.Add("logid", charger.logid);
                    charger.txdata = json.ToString((Newtonsoft.Json.Formatting)Formatting.None);
                    //charger.txdata = json.ToString();
        ...
        {
            charger.logid = DateTime.Now.ToString("yyyyMMddHHmmss") + "_diag";
        ...
        private void evcarUnlock()
        {
            var json = new JObject();
            json.Add("vendorId", "LGU");
            json.Add("messageId", "unlock");

            var json2 = new JObject();
            json2.Add("status", "Unlocked");
            json.Add("data", json2);
        ...
        private void button136_Click(object sender, EventArgs e)
        {
            if (svr.enrmtKeyId != string.Empty)
            {
                if (charger.entityId != string.Empty)
                {
                    ...
                    if (isRunning == true)
                        evTcpDisconnect();          // TCP connection
                    evTcpConnect();
                    if (isRunning == true)      // oneM2M EntityID read
                    {
                        SendingTCPStream("00", "{\"chargerid\":\"" + textBox14.Text + "\"}");
                    ...
                    charger.logid = DateTime.Now.ToString("yyyyMMddHHmmss") + "_admrsvd";
        ...
        {
            charger.logid = DateTime.Now.ToString("yyyyMMddHHmmss") + "_fota";
            evcarFOTA("Downloading");
        }

        private void evcarFOTA(string state)
        {
            var json = new JObject();
            json.Add("status", state);

            string retStr = SendDataToEVSP(fotaurl, json.ToString());
            //if (retStr != string.Empty)
            //    LogWrite(retStr);
        }
        ...
        private void evcarDiag(string state)
        {
            var json = new JObject();
            json.Add("status", state);

            string retStr = SendDataToEVSP(diaguri, json.ToString());
            //if (retStr != string.Empty)
            //    LogWrite(retStr);
        }
        ...
        {
            Console.WriteLine("Timer3 Ticked");
            ...
            switch (charger.nextstate)
            {
                ...
                case "meter":
                    ...
                    break;
                default:
                    Console.WriteLine("Timer3 Stoped");
                    timer3.Stop();
                    break;
            }
        }
        ...
        private void evcarMeter()
        {
            ...
            string retStr = SendDataToEVSP(meterVuri, json.ToString());
            //if (retStr != string.Empty)
            //    LogWrite(retStr);
        }

        private void evcarHeart()
        {
        ...
        private void button147_Click(object sender, EventArgs e)
        {
            var json = new JObject();
            json.Add("vendorId", "LGU");
            json.Add("messageId", "diagnostic");

            var json2 = new JObject();
            json2.Add("fileName", "test");
            json.Add("data", json2);
        ...
        {
            ListView.SelectedListViewItemCollection itemColl = listView4.SelectedItems;
            foreach (ListViewItem item in itemColl)
            {
                string data = item.SubItems[3].Text;
                data = data.Replace("&quot;", "\"");
    ...
    public class ReqHeader
    {
        ...
        public string X_EVC_RI { get; set; } // Request ID(동작 모드)
        public string X_EVC_BOX { get; set; } // 충전기 ID
    }
